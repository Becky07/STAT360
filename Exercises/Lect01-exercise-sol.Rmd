---
title: "Solutioins to exercises for lecture 01"
...
ee <- rlang::env(x=x,y=y)
# tracemem(ee) 
## error: tracemem() can't apply to an environment
ee$x <- 1:100
```
...
```{r}
# find size of x and y, one by one, with obj_size()
lobstr::obj_size(x)
lobstr::obj_size(y)
# output size of x and y simultaneously with obj_sizes(x,y)
# note that: obj_size(x,y) = obj_size(x)+obj_size(y)
lobstr::obj_sizes(x,y)
